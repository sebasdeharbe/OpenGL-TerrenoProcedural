	...
	do {
		...
		window.ImGuiDialog("Parametros Perlin",[&](){
			if(ImGui::InputInt("Tamanio Mapa", &parametros.tamanioMapa)) {
				if(parametros.tamanioMapa<8) parametros.tamanioMapa = 8; //Mínimo debe existir 1 octava.
				reload = true;
			}
			if(ImGui::InputInt("Semilla Rand",&parametros.seed)) reload = true;
			if(ImGui::InputInt("Frecuencia", &parametros.freq)){
				if(parametros.freq<0) parametros.freq = 0; //Mínimo debe existir 1 octava.
				reload = true;
			}
			if(ImGui::InputInt("Amplitud", &parametros.amp)){
				if(parametros.amp<0) parametros.amp = 0; //Mínimo debe existir 1 octava.
				reload = true;
			}
			if(ImGui::InputInt("Cantidad Octavas", &parametros.numeroDeOctavas)){
				if(parametros.numeroDeOctavas<1) parametros.numeroDeOctavas = 1; //Mínimo debe existir 1 octava.
				reload = true;
			}
